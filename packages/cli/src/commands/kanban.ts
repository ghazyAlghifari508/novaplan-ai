...
export async function kanbanCommand(projectId: string) {
  try {
    ...
    const maxLen = Math.max(cols.pending.length, cols.in_progress.length, cols.completed.length, cols.failed.length);
    ...
    console.log(`  ${chalk.gray("─".repeat(22))} ${chalk.gray("─".repeat(22))} ${chalk.gray("─".repeat(22))} ${chalk.gray("─".repeat(22))}`);
    ...
    for (let i = 0; i < maxLen; i++) {
      ...
      const pName = p ? chalk.gray(pad(p.name.slice(0, 20), 20)) : chalk.gray(pad("", 20));
      const ipName = ip ? chalk.blue(pad(ip.name.slice(0, 20), 20)) : chalk.gray(pad("", 20));
      const cName = c ? chalk.green(pad(c.name.slice(0, 20), 20)) : chalk.gray(pad("", 20));
      const fName = f ? chalk.red(pad(f.name.slice(0, 20), 20)) : chalk.gray(pad("", 20));
