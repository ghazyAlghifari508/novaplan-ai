...
interface TaskResponse {
  ...
  subtasks: Array<{ name: string; description: string; status: string; details: string[] }>;
}
...
export async function taskListCommand(projectId: string, options: { status?: string }) {
  try {
    ...
    for (const t of data.tasks) {
      ...
      if (t.description) {
        console.log(`  ${chalk.dim("             ")} ${chalk.dim(t.description.slice(0, 80))}`);
      }
      if (t.subtasks.length > 0) {
        for (const s of t.subtasks) {
          const sc = s.status === "completed" ? chalk.green("✓") : s.status === "in_progress" ? chalk.blue("●") : chalk.gray("○");
...
export async function taskNextCommand(projectId: string) {
  try {
    ...
    if (data.tasks.length === 0) {
      console.log(chalk.green("✓ Semua task sudah selesai! Tidak ada task pending."));
      return;
    }
    ...
    console.log();
    console.log(chalk.dim(`  Mulai: novaplan task update ${task.id} --status in_progress`));
    console.log();
  } catch (err) {
    console.error(chalk.red(`Error: ${err instanceof Error ? err.message : err}`));
    process.exit(1);
