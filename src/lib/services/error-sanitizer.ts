...
export function sanitizeErrorForClient(error: unknown, context?: "ac"): string {
  ...
  if (context === "ac") {
    if (msg.includes("Respons kosong")) return "AI mengembalikan respon kosong. Silakan generate ulang.";
  ...
  if (msg.includes("rate") || msg.includes("429")) return "Terlalu banyak permintaan. Silakan tunggu sebentar.";

  if (msg.includes("pgrst") || msg.includes("PostgrestError") || msg.includes("insforge")) {
    return "Terjadi kesalahan pada server. Silakan coba lagi.";
  }

  if (msg.includes("Unauthorized") || msg.includes("401")) return "Sesi Anda telah berakhir. Silakan login kembali.";

  if (msg.includes("Semua model") || msg.includes("Limit") || msg.includes("Gagal")) return msg;
