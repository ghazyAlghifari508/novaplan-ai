...
const loadHistory = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const [prdRows, acRows] = await Promise.all([
    db
      .select({ projectId: prdVersions.projectId, content: prdVersions.content, version: prdVersions.version })
      .from(prdVersions)
      .where(inArray(prdVersions.projectId, ids))
      .orderBy(desc(prdVersions.version)),
    db
      .select({ projectId: acVersions.projectId, content: acVersions.content, version: acVersions.version })
      .from(acVersions)
