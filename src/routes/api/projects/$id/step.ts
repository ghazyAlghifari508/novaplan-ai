  ...
  server: {
    handlers: {
      POST: async ({ request, params }: { request: Request; params: { id: string } }) => {
        ...
        const [existing] = await db.select({ step: projects.step }).from(projects).where(and(eq(projects.id, projectId), eq(projects.userId, user.id))).limit(1);
        ...
        await db.update(projects).set({ step: next }).where(and(eq(projects.id, projectId), eq(projects.userId, user.id)));
