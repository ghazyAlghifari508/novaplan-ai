  ...
  server: {
    handlers: {
      POST: async ({ request }: { request: Request }) => {
        ...
        if (!acMarkdown) return Response.json({ error: "AC not found. Generate AC first." }, { status: 404 });
        ...
        const stream = new ReadableStream<Uint8Array>({
          async start(controller) {
            ...
            const safeError = (msg: string) => {
              if (eventDone || eventErrored) return;
              eventErrored = true;
              db.update(projects).set({ taskStatus: "pending" }).where(eq(projects.id, projectId)).catch((e) => console.error("task_status reset failed:", e));
