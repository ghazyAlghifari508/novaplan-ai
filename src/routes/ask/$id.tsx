...
export const Route = createFileRoute('/ask/$id')({
  ...
  head: ({ loaderData }) => ({ meta: [{ title: loaderData?.projectName || 'Question' }] }),
  component: AskPage,
  errorComponent: ({ error }) => {
    if (error.message === 'NOT_FOUND') {
      return <div className="p-10 text-center text-fog">Proyek tidak ditemukan.</div>
    }
    return <div className="p-10 text-center text-crimson">Gagal memuat halaman.</div>
  },
})
