...
export const Route = createFileRoute('/prd/share/$token')({
  ...
  head: ({ loaderData }) => ({ meta: [{ title: loaderData?.projectName || 'Shared PRD' }] }),
  ...
  errorComponent: () => (
    <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-(--bg-card)">
      ...
      <Link href="/" className="rounded-lg border border-(--border-subtle) px-4 py-2 text-sm hover:bg-(--bg-surface)">
        Kembali
      </Link>
    </div>
  ),
