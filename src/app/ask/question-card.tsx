...
export function QuestionCard({ question, options, answer, onAnswer }: QuestionCardProps) {
  ...
  return (
    ...
    >
      <h3 className="mb-4 font-inter text-base font-[510]" style={{ color: "var(--text-primary)" }}>
        {question}
      </h3>
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          className={cn(
            ...
            answer?.isCustom ? "btn-primary border-solid" : "text-fog hover:bg-white/5 hover:text-snow",
          )}
        >
          + Lainnya
        </button>
      ...
      {answer?.isCustom && !showCustomInput && (
        <p className="mt-3 font-inter text-sm text-fog">
          Jawabanmu: <span style={{ color: "var(--text-primary)" }}>{answer.value}</span>
        </p>
      )}
    </div>
  );
