...
import { SITEMAP_GENERATION_PROMPT } from "@/lib/prompts-sitemap";
import { checkRateLimit } from "@/lib/rate-limit";
import { getLatestPrdContent } from "@/lib/services/prd-service";
import { parseSitemapJson, saveSitemapTree } from "@/lib/services/sitemap-service";
 ...
 * Generate sitemap page tree from latest PRD content.
...
export async function POST(req: NextRequest) {
  ...
  // Update sitemap_status='generating'
  await insforge.database.from("projects").update({ sitemap_status: "generating" }).eq("id", projectId);

  const systemPrompt = `${SITEMAP_GENERATION_PROMPT}\n\n--- PRD CONTENT ---\n${prdContent}`;
  const messages: Array<{ role: "system" | "user" | "assistant"; content: string }> = [
    { role: "system", content: systemPrompt },
    { role: "user", content: "Generate the sitemap JSON based on the PRD above." },
  ];
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      const safeDone = async (extras: Record<string, unknown> = {}) => {
        ...
        eventDone = true;
        try {
          const tree = parseSitemapJson(extractJson(fullResponse));
          if (!tree) {
            emit({ type: "error", error: "AI menghasilkan JSON tidak valid. Coba lagi." });
            try { controller.close(); } catch {}
            return;
          }
          const saveResult = await saveSitemapTree(insforge, projectId, tree);
          if (!saveResult.success) {
            emit({ type: "error", error: saveResult.error || "Gagal menyimpan sitemap" });
            try { controller.close(); } catch {}
            return;
          }
          emit({ type: "done", sitemapTree: tree, ...extras });
        } catch (err) {
          console.error("saveSitemapTree failed:", err);
          emit({ type: "error", error: "Failed to save sitemap" });
        }
        try { controller.close(); } catch {}
      };
      ...
      const safeError = (msg: string) => {
        if (eventDone || eventErrored) return;
        eventErrored = true;
        // Reset sitemap_status (non-fatal)
        Promise.resolve(
          insforge.database
            .from("projects")
            .update({ sitemap_status: "pending" })
            .eq("id", projectId),
        )
          .then(({ error }: { error: unknown }) => {
            if (error) console.error("sitemap_status reset failed:", error);
          })
          .catch((e: unknown) => console.error("sitemap_status reset rejected:", e));
